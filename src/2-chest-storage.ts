class ChestStorage {
	...
	static init(playerId: string, position: [number, number, number]) {
		...
		api.setBlock(
			...
			position[1],
			position[2],
			`Iron Chest|meta|${rotation}`,
		);
		api.setStandardChestItemSlot(position, 0, "Stick", 1, playerId, {
	...
	static isStorage(x: number, y: number, z: number, playerId?: string) {
		if (api.getBlock(x, y, z).startsWith("Iron Chest")) {
			const item = api.getStandardChestItemSlot([x, y, z], 0);
	...
	static set(
		...
		if (0 < index && index <= 35) {
			api.setStandardChestItemSlot([x, y, z], index, "Stick", 1, playerId, {
				customDescription: JSON.stringify(value),
			});
		} else {
			api.sendMessage(playerId, "Storage.set: Invalid index.", {
				color: "red",
			});
		}
	}
	...
	static get(playerId: string, x: number, y: number, z: number, index: number) {
		if (0 < index && index <= 35) {
			const item = api.getStandardChestItemSlot([x, y, z], index);
			if (item?.attributes?.customDescription) {
				return JSON.parse(item?.attributes?.customDescription);
			}
		} else {
			api.sendMessage(playerId, "Storage.get: Invalid index.", {
				color: "red",
			});
		}
		return null;
	...
	static teardown(
		...
		for (let index = 0; index <= length; index += 1) {
			api.setStandardChestItemSlot([x, y, z], index, "Air", 0, playerId, {});
		}
	}

	static onPlayerAttemptOpenChest = (
		playerId: string,
		x: number,
		y: number,
		z: number,
		_isMoonstoneChest: boolean,
		_isIronChest: boolean,
	) => {
		if (api.getBlock(x, y, z) === "Chest") {
			if (ChestStorage.isStorage(x, y - 1, z)) {
				api.sendMessage(playerId, "Break it to find out what's inside!", {
					color: "gold",
				});
				return "preventOpen";
			}
